...
qplot(x=EVTYPE, y = totFATALITIES, data = datQ1, fill=EVTYPE, 
      ...
      labs(title = "Figure 1: Fatalities caused by severe weather") +
...
qplot(x=Event, y = EconomicDamage, data = datQ2, fill=Event, 
      geom="bar", stat="identity", xlab="Type of Event", ylab="Million Dollars") +
      labs(title = "Figure 2: Economic damage occassioned by weather events") +
theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0), legend.position = "none") + 
scale_fill_manual(values=cbPalette)
```
...
with(datTotTime, plot(Year, TotalEconomicDamage, pch = NA, type="l", 
                      ylab="Million dollars", main="Figure 3: Economic Damage by Weather events in U.S.A.(1996-2011)",
                      xlab = "Year"))
